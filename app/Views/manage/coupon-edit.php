			...
			<div class="container-fluid py-1 px-3">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb bg-transparent mb-14 pb-0 pt-1 px-0 me-sm-6 me-5">
						...
						<li class="breadcrumb-item text-sm text-dark active" aria-current="page">쿠폰 수정</li>
					</ol>
					<h6 class="font-weight-bolder mb-0">쿠폰 수정</h6>
				</nav>
					...
					<div class="card card-body mt-4 p-5">
						...
						<form method="post" name="couponForm" id="couponForm">
							...
							<div class="form-group row align-items-center">
								...
								<div class="col">
									<input class="form-control" type="text" value="<?= isset($coupon->coupon_name) ? htmlspecialchars($coupon->coupon_name) : '' ?>" id="couponName" placeholder="ex) 웰컴쿠폰팩 (3만원 할인)" required>
								</div>
							</div>
							...
							<div class="form-group row align-items-center">
								...
								<div class="col">
									<input type="number" class="form-control" value="<?= isset($coupon->discount_amount) ? intval($coupon->discount_amount) : '' ?>" name="discountAmount" id="discountAmount" placeholder="ex) 5000" required Numberonly />
								</div>
							</div>
							...
							<div class="form-group row align-items-center">
								...
								<div class="col">
									<input type="number" class="form-control" value="<?= isset($coupon->minimum_order_price) ? intval($coupon->minimum_order_price) : '' ?>" name="minimumOrderPrice" id="minimumOrderPrice" placeholder="ex) 50000" required Numberonly />
								</div>
							</div>
							...
							<div class="form-group row align-items-center">
								...
								<div class="col">
									<input type="number" class="form-control" value="<?= isset($coupon->total_issuance) ? $coupon->total_issuance : '' ?>" name="totalIssuance" id="totalIssuance" placeholder="ex) 1000" required Numberonly />
								</div>
							</div>
							...
							<div class="form-group row align-items-center">
								...
								<div class="col">
									<div class="form-check form-switch">
										<input class="form-check-input" type="checkbox" id="isActive" <?= isset($coupon->is_active) && $coupon->is_active ? 'checked' : '' ?>>
									</div>
								</div>
							</div>
							...
							<div class="form-group row align-items-center">
								...
								<div class="col">
									<div class="form-check form-switch">
										<input class="form-check-input" type="checkbox" id="showInCouponWallet" <?= isset($coupon->show_in_coupon_wallet) && $coupon->show_in_coupon_wallet ? 'checked' : '' ?>>
									</div>
								</div>
							</div>
							...
							<div class="form-group row align-items-center">
								...
								<div class="col">
									<input id="couponStartDate" class="form-control datepicker" placeholder="Please select date" type="text" onfocus="focused(this)" onfocusout="defocused(this)" value="<?= isset($coupon->start_date) ? $coupon->start_date : '' ?>">
								</div>
							</div>
							...
							<div class="form-group row align-items-center">
								...
								<div class="col">
									<input id="couponEndDate" class="form-control datepicker" placeholder="Please select date" type="text" onfocus="focused(this)" onfocusout="defocused(this)" value="<?= isset($coupon->end_date) ? $coupon->end_date : '' ?>">
								</div>
							</div>
	...
	<script>
		...
		flatpickr('#couponEndDate', {
			...
			onReady: function(selectedDates, dateStr, instance) {
				const defaultDate = instance.input.value;
				if (defaultDate) {
					endDate = instance.formatDate(new Date(defaultDate), "Y-m-d H:i:S");
				}
			},
			onChange: function(selectedDates, dateStr, instance) {
				if (selectedDates.length > 0) {
		...
		document.addEventListener("DOMContentLoaded", () => {
			...
			submitFormButton.addEventListener("click", async () => {
				...
				try {
					// API 호출
					const response = await fetch('/api/reward/discount-coupon/edit', {
						method: 'POST',
						headers: {
					...
					if (result.success) {
						alert("쿠폰을 수정했습니다.");
						window.location.href = '/manage/reward/discount-coupons';
					} else {
			...
			cancelFormButton.addEventListener("click", async () => {
				location.href = '/manage/reward/discount-coupons';
			});
		});
	</script>
