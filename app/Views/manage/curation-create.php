...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-14 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">큐레이션 생성</li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">큐레이션 생성</h6>
                </nav>
        ...
        <div class="container-fluid py-4">
                    ...
                    <div class="row">
                        <div class="col-12 mx-auto">
                            <div class="card card-body p-5">
                                <h6 class="mb-0">신규 큐레이션 생성</h6>
                                <!-- <p class="text-sm mb-0">새로운 요금제 생성을 위해 기본 설정 및 부가 설정을 입력해 보세요</p> -->
                                <hr class="horizontal dark my-3">

                                <div>
                                    <div class="form-group row align-items-center">
                                        <label for="curationName" class="form-control-label col-sm-2">큐레이션명 <b class="text-danger">*</b></label>
                                        <div class="col">
                                            <input type="text" class="form-control" id="txtCurationTitle" name="txtCurationTitle" placeholder="노출되는 큐레이션 이름을 입력해 주세요 (한 줄)">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row align-items-center">
                                        <label for="curationDescription" class="form-control-label col-sm-2">큐레이션 설명 <b class="text-danger">*</b></label>
                                        <div class="col">
                                            <input type="text" class="form-control" id="txtCurationDescription" name="txtCurationDescription" placeholder="큐레이션명 하단에 노출되는 설명을 입력해 주세요">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row align-items-center">
                                        ...
                                        <div class="col">
                                            ...
                                            <div class="d-flex align-items-center gap-4">
                                                <input class="form-control w-35" type="datepicker" id="txtCurationVisibleFrom" name="txtCurationVisibleFrom">
                                                <span class="d-inline-block text-xs"> 부터 </span>
                                                <input class="form-control w-35" type="datepicker" id="txtCurationVisibleTo" name="txtCurationVisibleTo">
                        ...
                        <!-- 숙소 입력 -->
                        <div class="addStayWrap mt-5" id="divPartners">
                            <!-- S : 숙소(N개) -->
                            <!-- E : 숙소(N개) -->
                        </div>
                    </div>
            ...
            <div class="d-flex justify-content-center mt-4">
                ...
                <button type="button" id="btnSubmitForm" name="submitForm" class="btn bg-gradient-primary ms-2 mb-0 text-white" title="저장하기">생성하기</button>
            </div>
    ...
    <script>
		...
		flatpickr('#txtCurationVisibleFrom', {
			minDate: 'today',
			dateFormat: 'Y-m-d',
			enableTime: false,
			onReady: function(selectedDates, dateStr, instance) {
				...
				if (defaultDate) {
					startDate = instance.formatDate(new Date(defaultDate), "Y-m-d");
				}
			},
		...
		flatpickr('#txtCurationVisibleTo', {
			minDate: 'today',
			dateFormat: 'Y-m-d',
			enableTime: false,
			onReady: function(selectedDates, dateStr, instance) {
				const defaultDate = instance.input.value;
				if (defaultDate) {
					endDate = instance.formatDate(new Date(defaultDate), "Y-m-d");
				}
			},
			onChange: function(selectedDates, dateStr, instance) {
				if (selectedDates.length > 0) {
					endDate = instance.formatDate(selectedDates[0], "Y-m-d");
				}
			}
		});
...
$(function() {
    ...
    //숙소 1
    Tmpl.insert('#divPartners', '#tmplPartnerForm', {num : stayCounter, tags : TAG_CATALOG});
    initDropzone(document.querySelector('#targetThumbnail1'));

    //숙소 추가
    $('#btnAddStayBtn').off('click').on('click', function() {
        stayCounter++;
        Tmpl.append('#divPartners', '#tmplPartnerForm', {num : stayCounter, tags : TAG_CATALOG});
    ...
    //숙소 삭제
    $('#divPartners').on('click', '.btnDeletePartner', function () {

        var $el = $(this);
        var $citem = $el.closest('.stayAccordion');

        var total = $('#divPartners .stayAccordion').length;
        if (total <= 1) {
    ...
    function renumberStays() {
        $('#divPartners .stayAccordion').each(function(i) {
    ...
    // 태그 체크박스 선택/해제 시
    $('#divPartners').on('change', '.stay-tag-checkbox', function() {
        const $checkbox = $(this);
    ...
    // 태그 직접입력 (엔터 키)
    $('#divPartners').on('keydown', 'input[id^="tagCustomEnter"]', function(e) {
        if (e.key === 'Enter') {
    ...
    // 태그 직접입력 (+ 버튼 클릭)
    $('#divPartners').on('click', 'button[id^="btnTagCustomEnter"]', function() {
        const $input = $(this).siblings('input[id^="tagCustomEnter"]');
        handleCustomTagAdd($input);
    });

    // 미리보기 태그 삭제
    $('#divPartners').on('click', '.remove-tag-btn', function() {
    ...
    $('#btnSubmitForm').off('click').on('click', function() {
        ...
        var $stayAccordions = $('#divPartners .stayAccordion');
        ...
        Api.call({
            url : '/api/manage/postCuration', 
            data : data,
            success : function(res) {
                if (res.header.success) {
                    Msg.info('큐레이터 등록에 성공하였습니다. 목록에서 활성화시켜서 노출해주세요.', function() {
                        Comm.goTo('/manage/curations');
                    });
                } 
            }
