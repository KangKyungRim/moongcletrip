...
class ReservationViewController
{
    public static function reservations()
    {
        ...
        if(empty($_GET['searchAll'])) {
            $sql = "
                SELECT SQL_CALC_FOUND_ROWS
                    p.payment_status AS payment_status_main,
                    pi.payment_status AS payment_item_status,
                    p.*,
                    pi.*,
            ...
            $sql = "
                ...
                LEFT JOIN moongcletrip.payment_items pi ON pi.payment_idx = p.payment_idx 
                LEFT JOIN moongcletrip.partners partner ON partner.partner_idx = pi.partner_idx
                WHERE p.payment_status IN ('paid', 'canceled', 'partial_canceled', 'pending')
            ";
        }

        if(!empty($_GET['thirdparty'])) {
            $sql .= " AND partner.partner_thirdparty = '" . $_GET['thirdparty'] ."'";
        }
        ...
        self::render('manage/reservations', ['data' => $data]);
    }
