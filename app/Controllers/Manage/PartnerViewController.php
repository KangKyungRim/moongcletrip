...
class PartnerViewController
{
    public static function selectPartner()
    {
        ...
        $thirdparty = !empty($_GET['thirdparty']) ? trim($_GET['thirdparty']) : null;

        $data = PartnerHelper::adminDefaultProcess();

        $countQuery = Partner::query();
        if (!empty($thirdparty)) {
            $countQuery->where('partner_thirdparty', $thirdparty);
        }
        if (!empty($search)) {
        ...
        $query = Partner::query();
        if (!empty($thirdparty)) {
            $query->where('partner_thirdparty', $thirdparty);
        }
        if (!empty($search)) {
            $cleanedSearch = str_replace(' ', '', $search);
        ...
        self::render('manage/partner-select', ['data' => $data]);
    }
    ...
    public static function basicInfo()
    {
        ...
        if ($basicData['selectedPartnerIdx'] == -1) {
            self::render('manage/partner-basic-info', ['data' => $basicData]);
        ...
        self::render('manage/partner-basic-info', ['data' => $data]);
    }
    ...
    public static function editBasicInfo()
    {
        ...
        self::render('manage/partner-basic-info-edit', ['data' => $data]);
    }
    ...
    public static function basicInfoCreate()
    {
        ...
        self::render('manage/partner-basic-info-create', ['data' => $data]);
    }
    ...
    public static function curatedImages()
    {
        ...
        self::render('manage/curated-images', ['data' => $data]);
    }

    public static function partnerExposure()
    {
        $data = PartnerHelper::adminDefaultProcess();
        ...
        $withChildIdxArray = MainViewList::where('list_type', 'with_child')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $withSwimmingIdxArray = MainViewList::where('list_type', 'with_swimming')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $withNatureIdxArray = MainViewList::where('list_type', 'with_nature')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $aroundBeachIdxArray = MainViewList::where('list_type', 'around_beach')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $characterRoomIdxArray = MainViewList::where('list_type', 'character_room')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $kidsPensionIdxArray = MainViewList::where('list_type', 'kids_pension')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $costEffectiveIdxArray = MainViewList::where('list_type', 'cost_effective')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $glampingIdxArray = MainViewList::where('list_type', 'glamping')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $hanokExperienceIdxArray = MainViewList::where('list_type', 'hanok_experience')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $havingLargeRoomIdxArray = MainViewList::where('list_type', 'having_large_room')
            ->orderBy('list_order', 'ASC')
            ->pluck('partner_idx')
            ->toArray();

        $data['realtime_popularity'] = $popularityIdxArray;
        $data['with_child'] = $withChildIdxArray;
        $data['with_swimming'] = $withSwimmingIdxArray;
        $data['with_nature'] = $withNatureIdxArray;
        $data['around_beach'] = $aroundBeachIdxArray;
        $data['character_room'] = $characterRoomIdxArray;
        $data['kids_pension'] = $kidsPensionIdxArray;
        $data['cost_effective'] = $costEffectiveIdxArray;
        $data['glamping'] = $glampingIdxArray;
        $data['hanok_experience'] = $hanokExperienceIdxArray;
        $data['having_large_room'] = $havingLargeRoomIdxArray;

        self::render('manage/partner-exposure', ['data' => $data]);
    }
