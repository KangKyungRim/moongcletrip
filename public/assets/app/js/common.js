...
$(function () {
    setTimeout(
        function () {
            ...
            safeArea.left = parseInt(
                getComputedStyle(document.documentElement).getPropertyValue(
                    "--safeLeft"
                )
            );
            safeArea.header = parseInt(
                getComputedStyle(document.documentElement).getPropertyValue(
                    "--headerH"
                )
            );
            safeArea.filter = parseInt(
                getComputedStyle(document.documentElement).getPropertyValue(
                    "--filterH"
                )
            );
...
var registUI = function () {
    if ($(".container__wrap .fnStickyTop").length) {
        setTimeout(function () {
            fnStickyTop.set();
        }, 200);
    } // 스크롤에 따른 상단 sticky 고정
    if ($(".layerpop__wrap").length) {
        _layerPop();
        exeTransitionInLayer();
    } // 레이어팝업
    // if ( $('.check-list__wrap .check-list__all').length ) { _agreeCheckAll(); } // 전체동의
    if ($(".tab__wrap").length) {
        fnTabControl();
    } // 탭 기능
    if ($(".accordion__wrap").length) {
        fnAccorControl();
    } // 아코디언 기능
    if ($(".accordion__wrap_custom").length) {
        fnAccorControlCustom();
    } // 아코디언 기능
    if ($(".select__wrap").length) {
        fnSelectBtn();
    } // 셀럭트버튼
    // if ( $('.count__wrap').length ) { fnCountBtn(); } // 카운트 기능
    if ($(".splide__product").length) {
        fnProductSlide();
    }
    if ($(".splide__within__product").length) {
        fnProductWithinSlide();
    }
    if ($(".splide__point").length) {
        fnPointSlide();
    } // 뭉클포인트 슬라이드
    if ($(".splide__moongcle").length) {
        fnMoongcleSlide();
    } // 뭉클포인트 슬라이드
    if ($(".splide__close").length) {
        fnProductCloseSlide();
    } // 객실 마감 슬라이드
    if ($(".splide__default").length) {
        fnDefaultSlide();
    } // 디폴트 슬라이드
    if ($(".filter-select__wrap").length) {
        fnFilterBtn();
    } // 정렬 필터
    if ($(".rating").length) {
        fnRating();
...
var addNotchMeta = function () {
    ...
    if (isiOS) {
        $viewport.attr(
            "content",
            "width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, shrink-to-fit=no, viewport-fit=cover"
        );
        $viewport.after(
...
var fnStickyTop = (function () {
    var stickyEl, container, setHeight, top;
    ...
    function set() {
        ...
        if (
            !container ||
            container != $(stickyEl).parents(".content_layer")[0]
        ) {
            _getTop();
        }
        ...
        $(document).ready(function () {
            var $scrollTarget = $("#scrollContainer");
            if ($scrollTarget.length === 0) {
                $scrollTarget = $(window);
            }

            var $fnStickyTop =
                $(".fnStickyTop").offset().top -
                $(".header__wrap").height() +
                1;

            $scrollTarget.scroll(function () {
                var $winScrollTop = $scrollTarget.scrollTop();

                if ($winScrollTop >= $fnStickyTop) {
                    $(".fnStickyTop").addClass("fixed-top");
                } else {
                    $(".fnStickyTop").removeClass("fixed-top");
                }
            });
        });
    }
...
var _agreeCheckAll = function () {
    $(".check-list__wrap").each(function () {
        ...
        var _checkedAll = function () {
            // 전체체크
            if (
                $this.find('.check-list__con input[type="checkbox"]:checked')
                    .length == chks.length
            ) {
    ...
    $(".tab-sub__wrap .tab-sub__inner").each(function () {
        ...
        $tabBtn2.on("click", function () {
            ...
            var idx2 = $(this).closest("li").index(),
                $tabCont2 = $(this)
                    .parents(".tab-sub__wrap")
                    .find(".tab-subcontents");
...
var fnAccorControl = function () {
    $(".accordion__wrap").each(function () {
        ...
        $accBtn.on("click", function () {
            ...
            if ($("#gotoNextStepContainer").length) {
...
var fnAccorControlCustom = function () {
    $(".accordion__wrap_custom").each(function () {
        var $accBtn = $(this).find(".accordion__tit");
        ...
        $accBtn.on("click", function () {
            ...
            var $accorCont = $(this).siblings(".accordion__con_custom");
            $accorCont.slideToggle("100");
        });
    });
};
...
var fnFilterBtn = function () {
    $(".filter-select__wrap").each(function () {
        ...
        var $filterList = $this.find(".filter-select__list");
...
var fnLikeBtn = function () {
    $(document).on("click", ".btn-product__like", function () {
        ...
        $("#toastPopupLike p").text(message);
        fnToastPop("toastPopupLike");
    });
};

/**
 * @name fnPointSlide();
 * @description moongcle point slide
 */
var fnPointSlide = function () {
    $(".splide__point").each(function (index, element) {
        new Splide(element, {
            arrows: false,
            type: "loop",
            perPage: 1,
...
/**
 * @name fnMoongcleSlide();
 * @description 뭉클딜 slide
 */
var fnMoongcleSlide = function () {
    $(".splide__moongcle").each(function (index, element) {
        ...
        // 슬라이드 로드 및 이동시마다 높이를 자동으로 재조정
        splide.on("mounted move", function () {
            adjustSlideHeight(splide, element);
        });

        // 슬라이드 높이 자동 조정 함수
        function adjustSlideHeight(splide, element) {
            // 현재 활성화된 슬라이드의 높이를 가져와서 슬라이드 컨테이너의 높이를 조정
            var currentSlide = splide.Components.Elements.slides[splide.index];
            // var childElement = $(currentSlide).find('.box-gray__wrap').get(0);
            var slideHeight = currentSlide ? currentSlide.offsetHeight : 0;

            // 슬라이드 컨테이너 높이를 설정
            $(element).css("height", slideHeight + "px");

            // 페이지네이션 위치 재조정
            var pagination = $(element).find(".splide__pagination");
            pagination.css("bottom", -22 + "px");
        }
...
/**
 * @name fnProductCloseSlide();
 * @description product close slide / 객실 마감 슬라이드
 */
var fnProductCloseSlide = function () {
    $(".splide__close").each(function (index, element) {
        var splide = new Splide(element, {
            arrows: false,
            type: "loop",
            perPage: 1,
        ...
        function adjustSlideHeight(splide, element) {
            // 현재 활성화된 슬라이드의 높이를 가져와서 슬라이드 컨테이너의 높이를 조정
            var currentSlide = splide.Components.Elements.slides[splide.index];
            var childElement = $(currentSlide).find(".room-list__con").get(0);
            var slideHeight = childElement ? childElement.offsetHeight : 0;
...
var fnProductSlide = function () {
    $(".splide__product").each(function (index, element) {
        ...
        var totalSlides = $(element).find(".splide__list__product").length;
...
/**
 * @name fnProductWithinSlide();
 * @description 객실 마감 슬라이드
 */
var fnProductWithinSlide = function () {
    $(".splide__within__product").each(function (index, element) {
        var splide = new Splide(element, {
            arrows: true,
            perPage: 1,
            pagination: false,
...
$(document).ready(function () {
    ...
    function fillColor() {
        ...
        sliderTrack.css(
            "background",
            `linear-gradient(to right, #EBEBEB ${percent1}% , #714CDC ${percent1}% , #714CDC ${percent2}%, #EBEBEB ${percent2}%)`
        );
    }
});
...
document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll(".overflow-x-visible").forEach((el) => {
        el.addEventListener(
            "wheel",
            (e) => {
                // 세로 스크롤을 가로 스크롤로 전환
                if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
                    ...
                    const speed = 3;
                    const nextLeft = el.scrollLeft + e.deltaY * speed;
